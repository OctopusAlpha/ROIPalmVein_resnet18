...
if __name__ == '__main__':
    # 指定session1文件夹的路径
    folder_path = './Origin_Palmvein_ROI/session1'
    # 指定session2文件夹的路径
    folder_path = './Origin_Palmvein_ROI/session2'
    ...
    for filename in files:
        if filename.endswith('.bmp'):
            ...
            new_num = num + 6000
            # 生成新的文件名
            new_filename = f'{new_num:05d}.bmp'
            # 重命名文件
    ...
    session1_path = './Origin_Palmvein_ROI/session1'
    session2_path = './Origin_Palmvein_ROI/session2'
    ...
    session1_files = sorted([f for f in os.listdir(session1_path) if f.endswith('.bmp')])
    session2_files = sorted([f for f in os.listdir(session2_path) if f.endswith('.bmp')])
